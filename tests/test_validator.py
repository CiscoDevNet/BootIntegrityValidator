...
class TestBootIntegrityValidator(object):
    ...
    def test_validate_show_platform_integrity_line_wrap(self):
        show_plat = open(self.path + "/test_files/isr4k_show_plat_int_line_wrap.txt", "r")
        kgv = open(self.path + "/test_files/example_kgv.json", "rb")
        bi = BootIntegrityValidator(known_good_values=kgv.read())
        with pytest.raises(BootIntegrityValidator.MissingInfo):
            bi.validate(show_platform_integrity_cmd_output=show_plat.read())
    ...
    def test_boot_0_version_not_found(self):
        ...
        kgv = open(self.path + "/test_files/example_kgv.json", "rb")
        bi = BootIntegrityValidator(known_good_values=kgv.read())
        with pytest.raises(BootIntegrityValidator.VersionNotFound):
            bi.validate(show_platform_integrity_cmd_output=show_plat.read())
    ...
    def test_validate_2099(self):
        ...
        with pytest.raises(BootIntegrityValidator.ProductNotFound):
            bi.validate(show_platform_sudi_certificate_cmd_output=show_plat_sudi.read(),
